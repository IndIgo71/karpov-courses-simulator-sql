...
WITH
    cte_user_actions_by_camp AS (
        ...
        WHERE
            ua.time::date >= '2022-09-01'
            AND ua.time::date < '2022-09-08'
    )
    ,
    cte_order_price_by_user as (
        SELECT
            concat('Кампания № ', ads_campaign) AS ads_campaign,
            user_id,
            order_id,
            SUM(price) as order_price
        FROM cte_user_actions_by_camp
        WHERE
            ads_campaign IN (1, 2)
            AND is_canceled = 0
        GROUP BY
...
SELECT
    ads_campaign,
    ROUND(AVG(avg_check_1), 2) AS avg_check 
FROM (
    SELECT
        ads_campaign,
        user_id,
        AVG(order_price) AS avg_check_1 
    FROM cte_order_price_by_user
    GROUP BY
        ads_campaign,
        user_id 
) t
