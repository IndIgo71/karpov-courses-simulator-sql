...
WITH
    cte_users_by_camp AS (
        SELECT
            user_id,
            MIN(time::date) OVER (PARTITION BY user_id) AS start_date,
            time::date AS date,
            CASE
                WHEN user_id IN (8804, 9828, 9524, 9667, 9165, 10013, 9625, 8879, 9145, 8657,
...
SELECT
    ...
    ROUND(
        COUNT(DISTINCT user_id)::DECIMAL / MAX(COUNT(DISTINCT user_id)) OVER (PARTITION BY start_date, ads_campaign),
        2
    ) AS retention
FROM cte_users_by_camp
WHERE
    ads_campaign in (1, 2)
